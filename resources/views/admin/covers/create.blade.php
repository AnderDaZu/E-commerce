...
<form action="{{ route('admin.covers.store') }}" method="post" enctype="multipart/form-data">
    ...
    <figure class="relative mb-4">
        <img src="{{ asset('app/imgs/no_image.png') }}" class="aspect-[3/1] object-cover object-center w-full rounded-md border-2 border-dashed border-opacity-60" alt="no image" id="imgPreview">
    ...
    <div class="mb-4">
        ...
        <x-input name="title" type="text" value="{{ old('title') }}" id="title" class="w-full" placeholder="Ingrese título de portada" />
    ...
    <div class="mb-4">
        ...
        <x-input name="start_at" type="date" value="{{ old('start_at', now()->format('Y-m-d')) }}" id="start_at" class="w-full" />
    ...
    <div class="mb-4">
        ...
        <x-input name="end_at" type="date" value="{{ old('end_at') }}" id="end_at" class="w-full" />
        <x-input-error for="end_at" class="mt-2" />
    </div>

    <div class="mb-6 flex space-x-4">
        <label class="flex items-center space-x-1 hover:cursor-pointer">
            <x-input class="mr-0.5 hover:cursor-pointer" type="radio" name="is_active" value="1" checked />
            <span>Activo</span>
        </label>
        <label class="flex items-center space-x-1 hover:cursor-pointer">
            <x-input class="mr-0.5 hover:cursor-pointer" type="radio" name="is_active" value="0" />
            <span>Inactivo</span>
        </label>
    </div>
